...
import { apiSearch } from '../../../apis/api-film'
    ...
    <div className="absolute top-[50px] left-0 right-0 bg-header rounded-lg shadow-lg">
      {!films.length ? (
        ...
        <div className="flex flex-col gap-3 max-h-[480px] pl-3 py-3 overflow-y-auto scrollbar-thin scrollbar-thumb-primary scrollbar-track-header">
          {films.map((item) => (
            <div key={item.id} onClick={() => navigate(`/${item.mediaType}/${item.id}`)} className="flex items-start gap-3 p-2 hover:bg-primary cursor-pointer">
          ...
          {totalResults > 20 && (
            <button type="button" onClick={props.handleSearchPage} className="sticky bottom-0 w-full px-3 py-1.5 bg-primary rounded-md hover:text-body shadow-lg">
              Mais resultados
            </button>
          )}
        </div>
