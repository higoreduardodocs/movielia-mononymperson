...
export default function Trailler() {
  ...
  return (
    <div className={`${!trailler ? SHOW : HIDE} fixed top-0 right-0 bottom-0 left-0 after:fixed after:top-0 after:right-0 after:bottom-0 after:left-0 after:bg-black after:opacity-90 ease-out duration-300 z-20`}>
      <Container className="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 z-30 w-full overflow-hidden">
        <div className="bg-header rounded-lg">
          <button onClick={handleHideTrailler} type="button" className="p-3"><IoIosClose size={18} /></button>
